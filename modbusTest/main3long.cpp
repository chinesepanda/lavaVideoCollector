...
unsigned long nExp_us;//曝光时间（35~2260000us）
long paraForSet = 0;
...
char dataForRead[9] = "03010001";//读取输入电流的大小：0300寄存器地址，0001寄存器数量【不可更改】
...
{   
	...
	/*------------------------------------------------------------------------------------------------------------*/
	/*------------------------------------------------------操作台-------------------------------------------------*/
	strcpy(argv[1],"6000");//曝光时间（35~2260000us）[1]
	string plus = "(1)";//模式3专用
	/*--------记录-------------------------------------*/
	strcpy(argv[2],"3.5");//流股直径量度（流速）0-10
	strcpy(argv[3],"030404FF");//(暂时无用)下落点坐标报文，与模块输出电流有关(0-4095)0000-0FFF
	strcpy(argv[4],"6.5");//下落点量度 0-10
	
	int mod = 3;//图像采集模式 1.2.3
	/*------------------------------------------------------------------------------------------------------------*/
	string mfile;
	if( mod == 1)
	{
	...
	tmp << dOutputValue;
	tmp >> sOutputValue;
	csOutputValue = sOutputValue.c_str();//string转CString,不用
	//第四个参数
	CString csDropLocation;
	char2CString(argv[4], csDropLocation);
	
	//②生成视频名称
	const string file = "D:\\workspace\\data\\";//视频存储路径
	string a1 = argv[1] ;string a2 = argv[2] ;string a3 = argv[4] ;//中间变量
	string video_name;
	string video_name_time;
	if(mod == 3)
	{
		video_name = a1 +"_" + a2 +"_" + a3 + plus + ".avi" ;//视频名称：曝光时间_流速（流股直径）_输出电流值（下落点坐标）
		video_name_time = a1 +"_" + a2 +"_" + a3 + plus + "(time).avi" ;//时间戳视频名称：曝光时间_流速（流股直径）_输出电流值（下落点坐标）
	}
	else
	{
		video_name = a1 +"_" + a2 +"_" + a3 + ".avi" ;//视频名称：曝光时间_流速（流股直径）_输出电流值（下落点坐标）
		video_name_time = a1 +"_" + a2 +"_" + a3 + "(time).avi" ;//时间戳视频名称：曝光时间_流速（流股直径）_输出电流值（下落点坐标）
	}
	...
	if (mod ==3)
	{
		 excel_name = a1 +"_" + a2 +"_" + a3 + plus + ".xlsx";//要生成的excel文件名
	}
	else
	{
		excel_name = a1 +"_" + a2 +"_" + a3 + ".xlsx";//要生成的excel文件名
	}
	...
	string textHead[5] = {"记录时间","流股直径","流股厚度（空）","流股落点","产量信号（mA）"};//生成表头
	...
	VideoWriter outputVideo(video_file,CV_FOURCC('D', 'I', 'V', 'X'),/*帧率*/3.5,size,1/*彩色1黑白0*/);
	VideoWriter outputVideo_time(video_file_time,CV_FOURCC('D', 'I', 'V', 'X'),/*帧率*/3.5,size,1/*彩色1黑白0*/);
	cout<<"视频分辨率："<<w<<"x"<<h<<endl;
	cout<<"曝光时间："<<nExp_us<<endl;
	cout<<"输出帧率："<<fps<<endl;
	...
	while(true)
	{
		...
		if((iNumFrame % 50) == 0)//每1帧在表格储存一次数据
		{
			...
			excelProcessor.SetCellString(excel_y,4,csDropLocation);
			//读取此刻的输入电流
			int isRead = readModbusInput(uDataForRead);//读取此刻的输入电流值（产量信号）
			//double dOutputValue = ((double)iFallingPosition * 16.0 / 4096.0) + 4.0;/
		...
		m_cvDst = m_cvSrc.clone();	//获得屏幕输出的Mat ：m_cvDst
		
		//截图程序
		/*
		stringstream tmpn;
		string filename;
		tmpn << iNumFrame;//数字转中间变量
		tmpn >> filename;//中间变量转string
		filename = filename + ".jpg";
		filename = "C:\\Users\\Administrator\\Desktop\\12\\" + filename;
		imwrite(filename,m_cvDst);
		*/
...
int __stdcall StreamCB(MV_IMAGE_INFO *pInfo,long nUserVal)//MV回调函数,将MV格式数据转换为MAT
{
	//获取图像的尺寸
	...
	{
		//MVBayerToBGR(m_hCam,pInfo->pImageBuffer,m_cvSrc.data,w*3,w,h,m_PixelFormat);//不翻转时，直接把数据转为Mat

		MVBayerToBGR(m_hCam,pInfo->pImageBuffer,m_image.GetBits(),m_image.GetPitch(),w,h,m_PixelFormat);//buffer->MVImage
		MVImageFlip(m_hCam,&m_image,&m_image,FlipBoth);//翻转
		memcpy(m_cvSrc.data,m_image.GetBits(),w*h*3);
	}
	return 0;
}
